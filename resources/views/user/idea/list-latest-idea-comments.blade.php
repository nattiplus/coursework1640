...
<div class="tab-content" id="myTabContent">
    <div class="tab-pane fade show active" id="latest-comments" role="tabpanel" aria-labelledby="latest-comments-tab">
        Latest Comments
    </div>
</div>
@else
            ...
            ?>
                    <div class="container">
                        <ul class="nav nav-pills mt-3">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" id="btn_ideas_latest-comments" data-url="{{ route('list.ideas.latest-comments') }}" href="{{ route('list.ideas.latest-comments') }}">Latest Comments</a>
                            </li>
                        </ul>
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="latest-comments" role="tabpanel" aria-labelledby="latest-comments-tab">
                                <?php
                                // List all Idea
                                $ideas_latest_comment = App\Models\Idea::all();
                                // Array Store Latest Comment of Idea
                                $data = array();
                                $data_latest_comment = array();
                                foreach ($ideas_latest_comment as $key => $idea) 
                                {
                                    // Find Comment in each idea If That Comment Exist
                                    if($idea->comments->isNotEmpty())
                                    {
                                        $arr_latest_comment_idea = array();
                                        // Find All Comment
                                        $comment_all = $idea->comments;
                                        // Loop Each Comment Inside that idea
                                        foreach ($comment_all as $key => $comment) {
                                            array_push($arr_latest_comment_idea, $comment->create_date);
                                        }
                                        // Sort Array By Latest Comment
                                        $latest_comment_idea = collect($arr_latest_comment_idea)->sortBy('create_date')->toArray();
                                        // Store Latest Comment Of That Idea Into Array
                                        $data['id'] = $idea->id;
                                        ...
                                        $data['Comment'] = $latest_comment_idea[0];
                                        if(!in_array($data, $data_latest_comment))
                                        {
                                            array_push($data_latest_comment, $data);
                                        }
                                    }
                                    else {
                                        // Store Latest Comment Of That Idea Into Array
                                ...
                                // Sort Array By Most Like
                                $most_comment_idea = collect($data_latest_comment)->sortBy('Comment')->reverse()->toArray();
                                $most_comment_idea_sort = array();
                                foreach ($most_comment_idea as $key => $idea) {
                                    array_push($most_comment_idea_sort, $idea);
                                }
                                $collection_latest_comment = collect($most_comment_idea_sort);
                                $paginate_latest_comment = App\Http\Controllers\IdeaController::paginate($collection_latest_comment, 5);
                            ?>
                                <table class="table-custom">
                                    ...
                                    <tbody>
                                    <?php
                                        foreach($paginate_latest_comment as $key => $idea)
                                        { 
                                    ...
                                    ?>
                                            <tr>
                                                ...
                                                <td data-label="Viewer">
                                                    <p><i class="fas fa-eye " style="color: black;"></i> {{ App\Models\Idea::find($idea['id'])->viewers->count() }}</p>
                                                </td>
                                                <td data-label="Like">
