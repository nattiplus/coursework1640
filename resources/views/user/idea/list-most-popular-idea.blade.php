...
<div class="tab-content" id="myTabContent">
    <div class="tab-pane fade show active" id="most-popular-ideas" role="tabpanel" aria-labelledby="most-popular-ideas-tab">
        <?php
            // List all Idea
            $ideas_popular = App\Models\Idea::where('isApprove',true)->get();
            // Array Store Most Like Idea And DisLike Idea
            $arr_most_like_idea = array();
            $data = array();
            foreach ($ideas_popular as $key => $idea) 
            {
                // Count Reaction in each idea If That Reaction Exist
                if($idea->reactions->isNotEmpty())
                {
                    // Count Like Of That Idea
                    $count_like = 0;
                    $count_dislike = 0;
                    // Lopp Each reaction inside that idea
                    $reactions = $idea->reactions;
                    foreach($reactions as $index => $reaction)
                    {
                        // Seperate Reaction "Like" and "Dislike" inside that idea
                        if($reaction->type_id == 1)
                        {
                            // echo 'Like <br>';
                            $count_like++;
                        ...
                        else if($reaction->type_id == 2)
                        {
                            // echo 'DisLike <br>';
                            $count_dislike++;
                            // Count DisLike and Push It Into Array
                            // Sort Array and Display It into Idea Page
                        }
                    }
                    ...
                    $data['Like'] = $count_like;
                    $data['Dislike'] = $count_dislike;

                    array_push($arr_most_like_idea, $data);
                }
                else {
                    $data['id'] = $idea->id;
                    $data['title'] = $idea->title;
            ...
            $most_like_idea = collect($arr_most_like_idea)->sortBy('Like')->reverse()->toArray();
            $most_like_idea_sort = array();
            foreach ($most_like_idea as $key => $idea) {
                array_push($most_like_idea_sort, $idea);
            }
            $collection_most_like = collect($most_like_idea_sort);
            $paginate_most_like = App\Http\Controllers\IdeaController::paginate($collection_most_like, 5);
        ?>
        <table class="table table-striped table-hover">
            ...
            <tbody>
            <?php
                foreach($paginate_most_like as $key => $idea)
                { 
            ...
            ?>
                    <tr>
                        ...
                        <td>
                            <p><i class="fas fa-thumbs-up fa-1x" style="color: green;"></i> {{ $idea['Like'] }}</p>
                        </td>
                        <td>
                             <p><i class="fas fa-thumbs-down fa-1x" style="color: red;"></i> 
                                <?php
                                    $count_like = 0;
                                    $count_dislike = 0;
            ...
            ?>
                    <div class="container">
                        <ul class="nav nav-pills mt-3">
                            ...
                            <li class="nav-item">
                            <a class="nav-link active" id="btn_ideas_most-popular-ideas" data-url="{{ route('list.ideas.most-popular-ideas') }}" href="{{ route('list.ideas.most-popular-ideas') }}">Most Popular Ideas</a>
                            </li>
                            <li class="nav-item">
